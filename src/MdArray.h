...
{
	template <const int depth, typename ...T> static _f_hybrid constexpr size_t prodr(void) {return 1;}
	template <const int depth, const int T, const int... Tss> static _f_hybrid constexpr size_t prodr(void) {return (depth==0)?T:T*prodr<depth-1, Tss...>();}
	template <const int depth, const int... Tss> static _f_hybrid constexpr size_t prod(void) {return (depth==0)?1:prodr<depth-1, Tss...>();}
	static constexpr _f_hybrid size_t size(void)
	{
		return prod<sizeof...(vals), vals...>();
	}
	template <const int depth, typename index> _f_hybrid static inline size_t idxR(index i)
	{
		static_assert(std::is_integral<index>::value, "Integral value for index required.");
		return i*prod<sizeof...(vals)-1, vals...>();
	}
	template <const int depth, typename index, typename... indices> _f_hybrid static inline size_t idxR(index i, indices... is)
	{
		static_assert(std::is_integral<index>::value, "Integral value for index required.");
		return prod<depth, vals...>()*i + idxR<depth+1>(is...);
	}
	template <typename... indices> static inline _f_hybrid size_t offset(indices... is)
	{
...
{
	BaseContainer data;
	_f_hybrid DataView(void){}
	template <class rhsType> _f_hybrid DataView(const rhsType rhs)
	{
		data = rhs;
	}
	template <typename... indices> _f_hybrid inline BaseType& operator () (indices... is)
	{
